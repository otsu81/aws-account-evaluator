...
class BotoFactory:
    ...
    BotoFactory().get_capability(
        boto3.client, boto3.Session(profile_name='default',
        'ec2', '123456789012', 'OrganizationsAdmin', region='us-east-1')
        )

    Parameters:
        boto3_capability (function):The boto3 capability to be returned,
        e.g. client or resource session (boto3.Session):The session to be used
        to generate the capability, very useful for using specific profile
        defined in CLI service_name (boto3.service): Any AWS service available
        in boto3, e.g. EC2, IAM, SQS...
